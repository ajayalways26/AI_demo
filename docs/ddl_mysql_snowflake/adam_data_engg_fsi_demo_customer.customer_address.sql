CREATE OR REPLACE TABLE adam_data_engg_fsi_demo.adam_data_engg_fsi_demo_customer.customer_address (
    ...
    address_line_2     VARCHAR(255)        NULL COMMENT 'Secondary address line (e.g., suite or building)',
    city               VARCHAR(64)         NOT NULL COMMENT 'City of the address',
    state              VARCHAR(64)         NULL COMMENT 'State or province of the address',
    postal_code        VARCHAR(16)         NULL COMMENT 'ZIP or postal code',
    ...
    valid_from         TIMESTAMP_NTZ       DEFAULT CURRENT_TIMESTAMP NULL COMMENT 'Start date and time from when the address is valid (in UTC)',
    valid_to           TIMESTAMP_NTZ       NULL COMMENT 'End date and time until which the address is valid (in UTC)',
    created_at         TIMESTAMP_NTZ       DEFAULT CURRENT_TIMESTAMP NULL COMMENT 'Timestamp when the record was created (in UTC)',
    created_by         VARCHAR(64)         DEFAULT 'system' NULL COMMENT 'Identifier of the user or system that created the record',
    updated_at         TIMESTAMP_NTZ       DEFAULT CURRENT_TIMESTAMP NULL COMMENT 'Timestamp when the record was last updated (in UTC)',
    updated_by         VARCHAR(64)         DEFAULT 'system' NULL COMMENT 'Identifier of the user or system that last updated the record',
