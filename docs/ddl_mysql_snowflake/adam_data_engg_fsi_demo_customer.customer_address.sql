CREATE OR REPLACE TABLE adam_data_engg_fsi_demo.adam_data_engg_fsi_demo_customer.customer_address (
    ...
    valid_from       TIMESTAMP_NTZ      NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Start date and time from when the address is valid (in UTC)',
    valid_to         TIMESTAMP_NTZ      NULL COMMENT 'End date and time until which the address is valid (in UTC)',
    created_at       TIMESTAMP_NTZ      NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp when the record was created (in UTC)',
    created_by       VARCHAR(64)        NULL DEFAULT 'system' COMMENT 'Identifier of the user or system that created the record',
    updated_at       TIMESTAMP_NTZ      NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp when the record was last updated (in UTC)',
    updated_by       VARCHAR(64)        NULL DEFAULT 'system' COMMENT 'Identifier of the user or system that last updated the record',
