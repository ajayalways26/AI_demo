CREATE OR REPLACE TABLE adam_data_engg_fsi_demo.adam_data_engg_fsi_demo_customer.customer_relationship (
    ...
    effective_to         TIMESTAMP_NTZ DEFAULT NULL COMMENT 'End date of the relationship, if terminated (UTC)',
    created_at           TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP COMMENT 'Record creation timestamp (UTC)',
    created_by           VARCHAR(64) DEFAULT NULL COMMENT 'User or system who created the record',
    updated_at           TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP COMMENT 'Last updated timestamp (UTC)',
    updated_by           VARCHAR(64) DEFAULT NULL COMMENT 'User or system who last updated the record',

    CONSTRAINT PK_customer_relationship PRIMARY KEY (relationship_id),
    CONSTRAINT FK_customer_relationship_customer FOREIGN KEY (customer_id)
        REFERENCES adam_data_engg_fsi_demo.adam_data_engg_fsi_demo_customer.customer (customer_id),
    CONSTRAINT FK_customer_relationship_related_customer FOREIGN KEY (related_customer_id)
        REFERENCES adam_data_engg_fsi_demo.adam_data_engg_fsi_demo_customer.customer (customer_id)
)
